...
var<uniform> vat_instance_uniform: VatInstanceState;
...
fn sample_normal_frame(frame_index: f32, uv_b: vec2<f32>, fallback_normal: vec3<f32>) -> vec3<f32> {
    ...
    return normalize(convert_source_to_bevy(encoded * 2.0 - vec3<f32>(1.0)));
}
...
fn vertex(vertex: Vertex) -> VertexOutput {
    ...
    let vat_instance = vat_instance_uniform;
    var proxy_normal = vec3<f32>(0.0, 0.0, 1.0);
#ifdef NORMAL_PREPASS_OR_DEFERRED_PREPASS
#ifdef VERTEX_NORMALS
    proxy_normal = vertex.normal;
#endif
#endif
